...
from server.model_with_generator.models import *
from server.model_with_generator.metrics import *
...
def predict_test(folder='../data/test/', dest='../data/predictions'):
    ...
    y_pred = model.predict_and_show_plot(X_test, verbose=1, batch_size=128)
...
def plot_test_samples():
    ...
    n_imgs = np.load('../data/test_masks.npy').cumsum()
    ...
    imgs = np.stack(imgs)
    masks = model.predict_and_show_plot(imgs, verbose=1)
...
def check_predictions(the_list, plot=False):
    ...
    y_val = np.load('../data/validate.npy')
    ...
    if len(the_list) > 10:
        y_pred = model.predict_and_show_plot(X_train, verbose=1, batch_size=128)
        print('Results on train set:')
        print('Accuracy:', numpy_dice(y_train, y_pred))

    else:
        y_pred = model.predict_and_show_plot(X_val, verbose=1, batch_size=128)
